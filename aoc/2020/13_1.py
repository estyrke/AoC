...
# bus with ID `5` departs from the sea port at timestamps `0`, `5`,
# `10`, `15`, and so on. The bus with ID `11` departs at `0`, `11`,
# `22`, `33`, and so on. If you are there when the bus departs, you can
...
# IDs in service are `7`, `13`, `59`, `31`, and `19`. Near timestamp
...
# minutes before it departs. Multiplying the bus ID by the number of
...
def solve(inp: TextIOWrapper):
    earliest_time = int(inp.readline())
    buses = [int(b) for b in inp.readline().strip().split(",") if b != "x"]

    earliest_id = 0
    earliest_depart_time = earliest_time + max(buses)
    for bus in buses:
        bus_depart_time = earliest_time - (earliest_time % bus) + bus
        if bus_depart_time < earliest_depart_time:
            earliest_depart_time = bus_depart_time
            earliest_id = bus
    # for line in inp.readlines():
    # lines = [l for l in inp.readlines()]

    return earliest_id * (earliest_depart_time - earliest_time)
