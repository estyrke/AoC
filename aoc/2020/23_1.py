...
# this process the value goes below the lowest value on any cup's label,
...
# should appear exactly once. In the above example, after 10 moves, the
# cups clockwise from `1` are labeled `9`, `2`, `6`, `5`, and so on,
# producing *`92658374`*. If the crab were to complete all 100 moves,
...
def solve(inp: TextIOWrapper):
    ...
    cups = [int(c) for c in inp.read().strip()]
    ...
    for round in range(100):
        ...
        # print(
        #    " ".join([f"({c})" if c == current else str(c) for i, c in enumerate(cups)])
        # )
        pick_up = list(pop_wrapped(cups, current_idx))
        ...
        cups[destination_idx:destination_idx] = pick_up
        current_idx = (cups.index(current) + 1) % len(cups)
        current = cups[current_idx]
    ...
    return "".join(
        [str(cups[i % len(cups)]) for i in range(start_idx, start_idx + len(cups) - 1)]
    )
