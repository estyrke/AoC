...
# and `ne`. A tile is identified by a series of these directions with
# *no delimiters*; for example, `esenee` identifies the tile you land on
# if you start at the reference tile and then move one tile east, one
# tile southeast, one tile northeast, and one tile east.
#
...
def solve(inp: TextIOWrapper):
    ...
    return len([v for v in black_tiles.values() if v])
