...
# [bitmask](https://en.wikipedia.org/wiki/Mask_(computing)) system in
# its initialization program. Although you don't have the correct
...
# written to memory: a `0` or `1` overwrites the corresponding bit in
...
# bit is overwritten with `0`, and the `64`s bit is overwritten with
# `1`.
#
...
# every address.) In the above example, only two values in memory are
# not zero - `101` (at address `7`) and `64` (at address `8`) -
...
def mask_value(value: int, mask: List[Optional[str]]):
    value_bits = []
    for i in range(36):
    ...
    res_bits = [mask[i] if mask[i] is not None else value_bits[i] for i in range(36)]
    return sum([b * 2 ** (35 - i) for i, b in enumerate(res_bits)])
...
def solve(inp: TextIOWrapper):
    ...
    for line in inp.readlines():
        ...
        if op == "mask":
            ...
            addr = int(op[4:-1])
            mem[addr] = mask_value(int(value), mask)
