...
# would create a single-instruction infinite loop, never leaving that
...
def solve(inp):
    instrs = [l.strip().split() + [False] for l in inp.readlines()]

    for i, changed_instr in enumerate(instrs):
        acc = 0
        ip = 0
        for instr in instrs:
            instr[2] = False
        old_instr = changed_instr[0]
        if changed_instr[0] == "acc":
            continue
        elif changed_instr[0] == "nop":
            instrs[i][0] = "jmp"
        elif changed_instr[0] == "jmp":
            instrs[i][0] = "nop"
