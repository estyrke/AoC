...
def match(rules: Mapping, rule_name: str, message: str, indent) -> Tuple[bool, str]:
    # print(f"{' ' * indent}match {rule_name} {message}")
    for alt in rules[rule_name]:
        if alt[0].startswith('"'):
            literal = alt[0][1:-1]
            if message.startswith(literal):
                rest = message[len(literal) :]
                # print(f"{' ' * indent}{rule_name} {message} -> True, {rest}")
                return True, rest

            # literal that doesn't match
            # print(f"{' ' * indent}{rule_name} {message} -> False")
            return False, ""
        else:
            rest = message
            all_match = True
            for seq in alt:
                is_match, rest = match(rules, seq, rest, indent + 2)
                if not is_match:
                    all_match = False
                    break
            if all_match:
    ...
    for line in inp.readlines():
        if ":" in line:
            ...
            for rule in rules.keys():
                is_match, rest = match(rules, rule, message, 0)
                # print(message, rule, is_match, rest)
                if is_match and rest == "":
                    answer += 1
                    break
    # print(rules)
    return answer
