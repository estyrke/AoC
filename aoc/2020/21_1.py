...
# example, none of the ingredients `kfcds`, `nhms`, `sbzzf`, or `trh`
# can contain an allergen. Counting the number of times any of these
...
def solve(inp: TextIOWrapper):
    ...
    possibly_allergenic = functools.reduce(
        operator.ior, possible_ingredients_per_allergen.values(), set()
    )

    safe_ingredients = {ing for ing in ingredients if ing not in possibly_allergenic}
    return sum([len(ings & safe_ingredients) for ings, _ in foods])
