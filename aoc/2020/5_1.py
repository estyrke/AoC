...
#
# Instead of [zones or
# groups](https://www.youtube.com/watch?v=oAHbLRjF0vo), this airline
# uses *binary space partitioning* to seat people. A seat might be
# specified like `FBFBBFFRLR`, where `F` means "front", `B` means
...
# `127`). Each letter tells you which half of a region the given seat is
# in. Start with the whole list of rows; the first letter indicates
# whether the seat is in the *front* (`0` through `63`) or the *back*
# (`64` through `127`). The next letter indicates which half of that
# region the seat is in, and so on until you're left with exactly one
# row.
#
