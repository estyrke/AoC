...
# the first time the number had been spoken, the `4`th number spoken is
# *`0`*.
...
# most recently on turns `5` and `2`. So, the `6`th number spoken is `5
# - 2`, *`3`*.
...
def solve(inp):
    ...
    ages = {num: len(spoken) - i for i, num in enumerate(inp)}

    while len(spoken) < 2020:
        if spoken[-1] not in spoken[:-1]:
            spoken.append(0)
        else:
            spoken.append(1 + list(reversed(spoken[:-1])).index(spoken[-1]))
    return spoken[2019]
