...
# `1` through `10` clockwise. Each player's *starting space* is chosen
# randomly (your puzzle input). Player 1 goes first.
#
# Players take turns moving. On each player's turn, the player rolls the
# die *three times* and adds up the results. Then, the player moves
...
# `2`, and `1`, they would move forward 5 times, to spaces `8`, `9`,
...
# out. This die always rolls `1` first, then `2`, then `3`, and so on up
# to `100`, after which it starts over at `1` again. Play using this
# die.
#
...
# ends. At this point, the losing player had `745` points and the die
# had been rolled a total of `993` times; `745 * 993 = *739785*`.
#
...
def solve(inp: TextIOWrapper):
    ...
    while True:
        rolls += 3
        starting[0] = ((starting[0] - 1 + die) % 10) + 1
        scores[0] += starting[0]
        # print("1", starting[0], scores[0])
        if scores[0] >= 1000:
            print(scores[1], rolls)
            return scores[1] * rolls
        rolls += 3
        die = (die - 1) % 10
        starting[1] = ((starting[1] - 1 + die) % 10) + 1
        scores[1] += starting[1]
        # print("2", starting[1], scores[1])
        if scores[1] >= 1000:
