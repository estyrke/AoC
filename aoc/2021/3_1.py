...
def solve(inp):
    bits = [{"0": 0, "1": 0} for _ in range(12)]
    for line in inp.readlines():
        new_bits = list(line.strip())
        for i, b in enumerate(new_bits):
            bits[i][b] += 1

    gamma_bits = [(0 if b["0"] > b["1"] else 1) for b in bits]
    epsilon_bits = [(0 if b["0"] < b["1"] else 1) for b in bits]

    gamma = sum([v * 2 ** (11 - i) for i, v in enumerate(gamma_bits)])
    epsilon = sum([v * 2 ** (11 - i) for i, v in enumerate(epsilon_bits)])
