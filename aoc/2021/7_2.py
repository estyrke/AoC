...
# outcome; the old alignment position (`2`) now costs `206` fuel
# instead.
#
...
def solve(inp):
    crabs = [int(l) for l in inp.readline().strip().split(",")]
    fuel = None

    fuel_cost = [sum(range(dist + 1)) for dist in range(max(crabs) + 1)]

    for i in range(max(crabs)):
        n_fuel = sum([fuel_cost[abs(i - c)] for c in crabs])
        if fuel is None or n_fuel < fuel:
            fuel = n_fuel
    return fuel
