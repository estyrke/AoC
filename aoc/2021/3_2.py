...
def solve(inp):
    ...
    def filter_common_bit(lines: list, bit: int, one_condition):
        bits = {"0": 0, "1": 0}

        for new_bits in lines:
            bits[new_bits[bit]] += 1

        selected_bit = "1" if one_condition(bits) else "0"
        selected_lines = [l for l in lines if l[bit] == selected_bit]
    ...
    for round in range(12):
        if len(oxygen_lines) > 1:
            oxygen_lines = filter_common_bit(
                oxygen_lines, round, lambda bits: bits["1"] >= bits["0"]
            )
        if len(co2_lines) > 1:
            co2_lines = filter_common_bit(
                co2_lines, round, lambda bits: bits["1"] < bits["0"]
            )

    oxygen_bits = [int(b) for b in oxygen_lines[0]]
    co2_bits = [int(b) for b in co2_lines[0]]

    oxygen = sum([v * 2 ** (11 - i) for i, v in enumerate(oxygen_bits)])
    co2 = sum([v * 2 ** (11 - i) for i, v in enumerate(co2_bits)])
