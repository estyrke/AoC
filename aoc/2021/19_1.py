...
# scanner in each of the three axes (`x`, `y`, and `z`) have their
# precise position determined relative to the scanner. However, scanners
# cannot detect other scanners. The submarine has automatically
# summarized the relative positions of beacons detected by each scanner
# (your puzzle input).
#
...
# beacons* that both scanners detect within the overlap. By establishing
# 12 common beacons, you can precisely determine where the scanners are
...
# of the `x`, `y`, and `z` axes. That is, one scanner might call a
# direction positive `x`, while another scanner might call that
...
# the perspective of the other scanner. In total, each scanner could be
# in any of 24 different orientations: facing positive or negative `x`,
# `y`, or `z`, and considering any of four directions "up" from that
# facing.
#
...
def solve(inp: TextIOWrapper):
    ...
    while len(scanner_maps) > 1:
        for i, m in scanner_maps.items():
            if i != 0:
                new_beacons = align(scanner_maps[0], m)
    ...
    return len(scanner_maps[0])


def align(m0: Set, m: Set):
    for rot in rotations(m):
        for p0 in m0:
            for shifted in shifts(rot, p0):
                overlap = m0 & shifted
                if len(overlap) >= 12:
                    return m0 | shifted
...
def y_rotations(m):
    yield m
    yield {(p[2], p[1], -p[0]) for p in m}
    yield {(-p[0], p[1], -p[2]) for p in m}
    yield {(-p[2], p[1], p[0]) for p in m}


def x_rotations(m):
    yield {(p[0], p[2], -p[1]) for p in m}
    yield {(p[0], -p[2], p[1]) for p in m}


def z_rotations(m):
    yield m
    yield {(p[1], -p[0], p[2]) for p in m}
    yield {(-p[0], -p[1], p[2]) for p in m}
    yield {(-p[1], p[0], p[2]) for p in m}


def rotations(m):
    for yrot in y_rotations(m):
        yield from z_rotations(yrot)

    for xrot in x_rotations(m):
        yield from z_rotations(xrot)
...
def shifts(m, p0):
    for pt in m:
        pd = point_sub(pt, p0)
        yield {point_sub(p, pd) for p in m}
