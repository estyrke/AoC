...
# has no regular number to its left, so it is not added to any regular
# number).
...
# numbers* (your puzzle input). The snailfish numbers are each listed on
# a separate line. Add the first snailfish number and the second, then
# add that result and the third, then add that result and the fourth,
...
def solve(inp: TextIOWrapper):
    ...
    res = functools.reduce(add_literal, nums)
    return magnitude_literal(res)
