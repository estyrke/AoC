...
# input). Each step specifies a
# [cuboid](https://en.wikipedia.org/wiki/Cuboid) (the set of all cubes
# that have coordinates which fall within ranges for `x`, `y`, and `z`)
...
#
# The initialization procedure only uses cubes that have `x`, `y`, and
# `z` positions of at least `-50` and at most `50`. For now, ignore
...
def solve(inp: TextIOWrapper):
    ...
    field = np.zeros((101, 101, 101))
    for line in inp.readlines():
        m = r.match(line.strip())
        assert m
        state = m.group(1)
        xmin, xmax, ymin, ymax, zmin, zmax = map(int, m.groups()[1:])
        if (
            xmin < -50
            or xmax > 50
            or ymin < -50
